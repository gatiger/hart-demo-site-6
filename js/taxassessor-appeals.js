...
async function initAppealsPage(){
  try{
    const res = await fetch("/content/taxassessor-appeals.json", { cache: "no-store" });
    if(!res.ok) throw new Error("Failed to load appeals JSON");
    ...
    console.error(err);
    setText("appealIntro", "Appeals information is unavailable right now. Please contact the Tax Assessor’s Office for assistance.");
  }
}

function renderHero(hero){
  setText("appealEyebrow", hero.eyebrow || "Tax Assessor");
  setText("appealPageTitle", hero.title || "Appeals Information");
  setText("appealIntro", hero.intro || "Information about how to appeal a property assessment.");

  const mount = document.getElementById("appealActions");
  if(mount){
...
function renderNotice(notice){
  const card = document.getElementById("appealNoticeCard");
  const title = document.getElementById("appealNoticeTitle");
  const body = document.getElementById("appealNoticeBody");
  ...
  card.hidden = false;
  title.textContent = noticeTitle || "Important Notice";
  body.innerHTML = noticeBody;
}

function renderProcess(process){
  setText("appealStepsTitle", process.title || "Appeal Process");
  setText("appealStepsIntro", process.intro || "");

  const list = document.getElementById("appealStepsList");
  if(!list) return;

  const steps = Array.isArray(process.steps) ? process.steps.filter(Boolean) : [];
  list.innerHTML = steps.map(step => `
    <li>
      <div class="appealStepContent">
...
function renderHelpfulDetails(details){
  setText("appealSideTitle", details.title || "Helpful Details");
  setText("appealSideIntro", details.intro || "");

  const mount = document.getElementById("appealSideBlocks");
  if(!mount) return;
  ...
  mount.innerHTML = blocks.map(block => {
    ...
    return `
      <section class="appealSideBlock" aria-label="${escapeAttr(block.title || "Appeal detail")}">
...
function renderInfoCards(cards){
  const mount = document.getElementById("appealInfoCards");
  if(!mount) return;
  ...
  mount.innerHTML = visible.map(card => {
    ...
    return `
      <section class="card appealInfoCard" aria-label="${escapeAttr(card.title || "Additional information")}">
        <div class="appealCardHead">
          ${safeText(card.title) ? `<h2>${escapeHtml(card.title)}</h2>` : ""}
          ${safeText(card.subtitle) ? `<p>${escapeHtml(card.subtitle)}</p>` : ""}
        </div>
        <div class="appealInfoBody">
          ${renderParagraphs(card.text)}
          ${items.length ? `<ul class="appealInfoList">${items.map(item => `<li>${escapeHtml(item)}</li>`).join("")}</ul>` : ""}
          ${actions.length ? `<div class="appealCardActions">${actions.map(renderAction).join("")}</div>` : ""}
        </div>
      </section>
    `;
  }).join("");
